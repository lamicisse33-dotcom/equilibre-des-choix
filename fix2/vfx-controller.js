...
export class VFXController {
    ...
    triggerAbilityVFX(abilityId) {
        const abilityColors = {
            'deep_meditation': 0x10b981, // Health green
            'negotiator': 0xf1c40f,      // Money gold
            'carpe_diem': 0xe74c3c,      // Love red
            'harmony_mastery': 0xffffff  // Pure white
        };

        const color = abilityColors[abilityId] || 0xc5a059;
        
        if (abilityId === 'harmony_mastery') {
            this.triggerCelestialFlash(); 
        } else if (abilityId === 'deep_meditation') {
            this.burstParticles(0x10b981, new THREE.Vector3(0, 2, 0), 100);
            this.burstParticles(0x4f46e5, new THREE.Vector3(0, 1, 0), 50);
        } else if (abilityId === 'negotiator') {
            this.burstParticles(0xf1c40f, new THREE.Vector3(-1, 1, -1), 80);
        } else if (abilityId === 'carpe_diem') {
            this.burstParticles(0xe74c3c, new THREE.Vector3(1, 1, -1), 80);
        } else {
            this.burstParticles(color, new THREE.Vector3(0, 1, 0), 40);
        }
    }

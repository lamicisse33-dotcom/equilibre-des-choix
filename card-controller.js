...
export class CardController {
    ...
    static CARD_W = 1.1;      // largeur d'une carte (BoxGeometry)
    static SLOT_X = 2.15;     // ecartement horizontal des emplacements lateraux
    ...
    constructor(scene, textures) {
        ...
        this.cardGeometry = new THREE.BoxGeometry(CardController.CARD_W, 1.6, 0.04);
    ...
    /**
     * Gabarit de la rangee de cartes en fonction du ratio d'ecran.
     * Continu (et non plus binaire portrait/paysage) : sur un telephone
     * 9:19.5 l'ancien palier laissait encore deborder les cartes laterales.
     * SceneManager consomme la meme fonction pour caler la camera.
     */
    static layoutFor(aspect) {
        if (aspect >= 1.0) return { spread: 1.0, scale: 1.0 };
        const t = Math.max(0, Math.min(1, (aspect - 0.42) / 0.58));
        return {
            spread: 0.56 + t * 0.44,
            scale: 0.82 + t * 0.18
        };
    ...
    /** Demi-largeur occupee par la rangee, marge de securite incluse. */
    static halfWidthFor(aspect) {
        const l = CardController.layoutFor(aspect);
        return CardController.SLOT_X * l.spread + (CardController.CARD_W * l.scale) / 2 + 0.28;
    }

    /**
     * Recalcule ecartement et echelle des cartes deja posees apres une
     * rotation d'ecran, sans les redessiner (les textures sont conservees).
     */
    relayout() {
        ...
        this.cards.forEach(mesh => {
            ...
            if (!base) return;
            mesh.userData.slot.set(base.x * layout.spread, base.y, base.z);
            mesh.userData.baseScale = layout.scale;
            if (!mesh.userData.isSelected) {
                mesh.position.x = mesh.userData.slot.x;
    ...
    createCards(cardData, slots) {
        ...
        cardData.forEach((data, i) => {
            ...
            mesh.userData = {
                ...
                data: data,
                slot: slot,
                baseSlot: baseSlot.clone(),  // position non compressee, pour relayout()
    ...
    drawCardToCanvas(canvas, data) {
        ...
        if (artTexture && artTexture.image) {
            ...
            let dw = artW, dh = artH;
            if (ratio > frameRatio) { dh = artW / ratio; } else { dw = artH * ratio; }
        ...
        ctx.font = `700 ${title.length > 16 ? 26 : 32}px Cinzel, serif`;
        ctx.fillText(title, W / 2, 500);
        ctx.restore();

        // Divider under title
        const dividerGrad = ctx.createLinearGradient(120, 0, 392, 0);
        ...
        this.wrapText(ctx, data.desc || '', W / 2, 548, 400, 26);

        // --- 7. EFFETS (+ / -) ---
        this.drawEffects(ctx, data, W, 636);
    ...
    drawEffects(ctx, data, W, y) {
        ...
        if (entries.length === 0) {
            // Les cartes a convergence n'ont pas d'effets chiffres : on annonce
            // la mecanique en clair plutot que de laisser le bas de carte vide.
            if (data.convergence) {
                ctx.textAlign = 'center';
                ctx.font = '700 17px Cinzel, serif';
                ctx.fillStyle = '#f3e5ab';
                ctx.fillText('✦ CONVERGENCE ✦', W / 2, y + 20);
                ctx.font = '400 14px Montserrat, sans-serif';
                ctx.fillStyle = 'rgba(243,229,171,0.75)';
                const pct = Math.round((data.convergence.ratio || 0) * 100);
                const bonus = data.convergence.bonus || 0;
                ctx.fillText(
                    bonus > 0 ? `Alignement ${pct}%  ·  +${bonus} à tous` : `Alignement ${pct}%`,
                    W / 2, y + 44
                );
                return;
            }
            if (data.specialEffect) {
                ...
                ctx.textAlign = 'center';
                ctx.fillStyle = '#f3e5ab';
                ctx.fillText('✦ EFFET SPÉCIAL ✦', W / 2, y + 20);
            }
            return;
        }

        const chipW = 92, chipH = 44, gap = 10;
        ...
        for (let r = 0; r < rows; r++) {
            ...
            for (const [pillar, value, perTurn] of rowItems) {
                ...
                ctx.save();
                this.roundRectPath(ctx, x, rowY, chipW, chipH, 8);
                ...
                if (icon && icon.image) {
                    ...
                    ctx.drawImage(icon.image, x + 12, rowY + (chipH - ih) / 2, iw, ih);
                }

                ctx.font = '700 20px Cinzel, serif';
                ctx.textAlign = 'right';
                ctx.fillStyle = positive ? '#63e6a2' : '#ff7a7a';
                const label = `${positive ? '+' : ''}${value}${perTurn ? '/t' : ''}`;
                ctx.fillText(label, x + chipW - 12, rowY + chipH / 2 + 7);
    ...
    /** Word-wrap helper for card description text (max 3 lines). */
    wrapText(ctx, text, centerX, startY, maxWidth, lineHeight) {
        ...
        capped.forEach((l, i) => ctx.fillText(l, centerX, startY + i * lineHeight));
    }
